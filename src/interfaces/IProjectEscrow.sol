...
interface IProjectEscrow {
    ...
    function createProject(address kuli, uint256 dailyRate, uint256 durationDays) external returns (uint256);

    function depositFunds(uint256 projectId) external payable;
    function startProject(uint256 projectId) external;

    // Functions - Kuli
    function submitWorkProof(uint256 projectId, string calldata photoHash, int256 latitude, int256 longitude)
        external
        returns (uint256);

    // Functions - Admin (AI Oracle)
    function verifyWorkProof(uint256 projectId, uint256 proofId, bool verified) external;

    // Functions - Dispute
    function raiseDispute(uint256 projectId, string calldata reason) external;
    function resolveDispute(uint256 projectId, bool favorKuli, uint256 amount) external;
